...
import org.linqs.psl.utils.textsimilarity.*
...
//! import org.linqs.psl.utils.evaluation.result.memory.MemoryFullInferenceResult
...
DatabaseDriver h2dDriver = new H2DatabaseDriver(Type.Disk, dbpath, true)
DataStore data = new RDBMSDataStore((DatabaseDriver) h2dDriver)
//this is the id for the new Partition
//int partitionID = 0;
Partition evidencePartitionCommon = data.getNewPartition();
...
for(int fold=1;fold<=numFolds;fold++){
	...
	Partition evidencePartitionWL1 = data.getNewPartition();// observed data for weight learning
	Partition targetPartitionWL1 = data.getNewPartition();// unobserved data for weight learning
	Partition trueDataPartitionWL1 = data.getNewPartition();  // train set for inference
	
	//this fold is for validating (finding the right threshold)
	Partition evidencePartitionValid = data.getNewPartition();		 // test set for validation
	Partition targetPartitionValid = data.getNewPartition();
	
	//this fold is for testing
	Partition evidencePartitionTest = data.getNewPartition();		 // test set for inference
	Partition targetPartitionTest = data.getNewPartition();
	
	if(fold==1){
		//load boolean data
		def insert = data.getInserter(person, evidencePartitionCommon)
		insert.loadDelimitedData(dir+"persons");
		
		println "About to load the belongsToFamily"
		insert = data.getInserter(belongsToFamily, evidencePartitionCommon)
		insert.loadDelimitedData(dir+"belongsToFamily");
		
		println "About to load the belongsToParticipant"
		insert = data.getInserter(belongsToParticipant, evidencePartitionCommon)
		insert.loadDelimitedData(dir+"belongsToParticipant");
		...
		insert.loadDelimitedDataTruth(namesDir +"firstNameJaroSims");
		
		insert = data.getInserter(maidenNameJaroSim, evidencePartitionCommon)
		insert.loadDelimitedDataTruth(namesDir+"maidenNameJaroSims");
		
		insert = data.getInserter(lastNameJaroSim, evidencePartitionCommon)
		insert.loadDelimitedDataTruth(namesDir+"lastNameJaroSims");
		...
		insert.loadDelimitedDataTruth(namesDir+"firstNameLevSims");
		
		insert = data.getInserter(maidenNameLevSim, evidencePartitionCommon)
		insert.loadDelimitedDataTruth(namesDir+"maidenNameLevSims");
		
		insert = data.getInserter(lastNameLevSim, evidencePartitionCommon)
		insert.loadDelimitedDataTruth(namesDir+"lastNameLevSims");
		...
		if(secDegSimRun){
			...
			insert.loadDelimitedDataTruth(relationalSimDir+"grandFathersSim");
			
			insert = data.getInserter(grandMothersSim, evidencePartitionCommon)
			insert.loadDelimitedDataTruth(relationalSimDir+"grandMothersSim");
			
			insert = data.getInserter(grandDaughtersSim, evidencePartitionCommon)
			insert.loadDelimitedDataTruth(relationalSimDir+"grandDaughtersSim");
			
			insert = data.getInserter(grandSonsSim, evidencePartitionCommon)
			insert.loadDelimitedDataTruth(relationalSimDir+"grandSonsSim");

			println "unclesSim file not found!!!!!!!!!!!!!!!"
			//! insert = data.getInserter(unclesSim, evidencePartitionCommon)
			//! insert.loadDelimitedDataTruth(relationalSimDir+"unclesSim");
			
			insert = data.getInserter(auntsSim, evidencePartitionCommon)
			insert.loadDelimitedDataTruth(relationalSimDir+"auntsSim");
			
			insert = data.getInserter(niecesSim, evidencePartitionCommon)
			insert.loadDelimitedDataTruth(relationalSimDir+"niecesSim");
			
			insert = data.getInserter(nephewsSim, evidencePartitionCommon)
			insert.loadDelimitedDataTruth(relationalSimDir+"nephewsSim");
		}
	...
	insert.loadDelimitedData(dir+"samePersonBlockWL1.fold"+fold);
	...
	insert.loadDelimitedDataTruth(dir + "samePersonTrainTruthWL1.fold" + fold);
	
	println "TIME after reading the files " + new Date();
	
	//call the function that performs weight learning
	performWeightLearning(m, data,
	...
	insert.loadDelimitedData(dir+"samePersonValidBlock.fold"+fold);
	...
	MPEInference inferenceApp = new MPEInference(m, dbValid);
	//! MemoryFullInferenceResult inf_result = inferenceApp.mpeInference();
	inferenceApp.inference();
	...
	insert.loadDelimitedData(dir+"toPredict.fold"+fold);
	...
	//run MPE inference with learned weights
	inferenceApp = new MPEInference(m, dbInf);
...
PSLModel createModel(Object object, DataStore data, 
	...
	idType = ConstantType.UniqueIntID

	//general predicates
	m.add predicate: "person", types: [idType]
	m.add predicate: "belongsToFamily", types:[idType,idType]
	m.add predicate: "belongsToParticipant", types:[idType,idType]
	
	//similarity predicates
	m.add predicate: "firstNameJaroSim", types:[idType,idType]
	m.add predicate: "maidenNameJaroSim", types:[idType,idType]
	m.add predicate: "lastNameJaroSim", types:[idType,idType]
	
	//this predicate is used for the transitive relational rules
	m.add predicate: "maxJaroLevFirstName", types:[idType,idType]
	
	m.add predicate: "firstNameLevSim", types:[idType,idType]
	m.add predicate: "maidenNameLevSim", types:[idType,idType]
	m.add predicate: "lastNameLevSim", types:[idType,idType]
	
	m.add predicate: "similarAge", types:[idType,idType]
	m.add predicate: "sameGender", types:[idType,idType]
	m.add predicate: "sameLivingStatus", types:[idType,idType]
	
	//relationship similarities equivalent to logistic regression ones
	//first degree individual
	m.add predicate: "motherSim", types:[idType, idType]	//total
	m.add predicate: "fatherSim", types:[idType, idType]	//total
	m.add predicate: "brothersSim", types:[idType, idType]	//total
	m.add predicate: "sistersSim", types:[idType, idType]	//total
	m.add predicate: "daughtersSim", types:[idType, idType]	//total
	m.add predicate: "sonsSim", types:[idType, idType]		//total
	m.add predicate: "spouseSim", types:[idType, idType]	//total
	
	//second degree individual
	m.add predicate: "grandMothersSim", types:[idType, idType]	//total
	m.add predicate: "grandFathersSim", types:[idType, idType]	//total
	m.add predicate: "grandDaughtersSim", types:[idType, idType]	//total
	m.add predicate: "grandSonsSim", types:[idType, idType]	//total
	m.add predicate: "unclesSim", types:[idType, idType]	//total
	m.add predicate: "auntsSim", types:[idType, idType]	//total
	m.add predicate: "niecesSim", types:[idType, idType]	//total
	m.add predicate: "nephewsSim", types:[idType, idType]	//total
	
	//predicates for 1st degree familial relations - these predicates are for the transitivity like rules
	m.add predicate: "hasMother", types:[idType, idType]	//total 5,087
	m.add predicate: "hasFather", types:[idType, idType]	//total 5,050
	m.add predicate: "hasBrother", types:[idType, idType]	//total 13,904
	m.add predicate: "hasSister", types:[idType, idType]	//total 13,121
	m.add predicate: "hasDaughter", types:[idType, idType]	//total 4,051
	m.add predicate: "hasSon", types:[idType, idType]	//total 4,118
	m.add predicate: "hasSpouse", types:[idType, idType]	//total 1,652
	
	//predicates for 2nd degree familial relations
	m.add predicate: "hasGrandMother", types:[idType, idType]	//total
	m.add predicate: "hasGrandFather", types:[idType, idType]	//total
	m.add predicate: "hasGrandDaughter", types:[idType, idType]	//total
	m.add predicate: "hasGrandSon", types:[idType, idType]	//total
	m.add predicate: "hasUncle", types:[idType, idType]	//total
	m.add predicate: "hasAunt", types:[idType, idType]	//total
	m.add predicate: "hasNiece", types:[idType, idType]	//total
	m.add predicate: "hasNephew", types:[idType, idType]	//total

	//this predicate is always observed and is used only for blocking!
	m.add predicate: "samePersonBlock", types: [idType, idType]
	//the predicate we are interested in predicting
	m.add predicate: "samePerson", types: [idType, idType]
...
void performWeightLearning(PSLModel m, DataStore data, 
	...
	MaxLikelihoodMPE weightLearning = new MaxLikelihoodMPE(m, dbWL, trueDataDB);
...
HashMap<String, Double>  loadPredictedValues(Database db){
	...
	for (GroundAtom atom : db.getAllGroundAtoms(samePerson)){
