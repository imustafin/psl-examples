...
readonly BASE_NAME='AllRulesValidTrainOn4FoldsNewThres'
readonly CLASSPATH_FILE='classpath.out'
readonly TARGET_CLASS="edu.ucsc.NIH.${BASE_NAME}"
...
function run() {
   ...
   java -cp ./target/classes:$(cat ${CLASSPATH_FILE}) ${TARGET_CLASS} -Xmx20g
   if [[ "$?" -ne 0 ]]; then
